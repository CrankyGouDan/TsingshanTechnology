...
import io.gitee.crankygoudan.tsingshantechnology.jieko.Constants;
 ...
 * @author DAIDAI
 */
public class Yuangujitanxiang extends SlimefunItem implements EnergyNetComponent {
    ...
    public Yuangujitanxiang(ItemGroup category, SlimefunItemStack item, RecipeType recipeType, ItemStack[] recipe) {
        super(category, item, recipeType, recipe);

        new BlockMenuPreset(getId(), "&5全自动远古祭坛箱") {
    ...
    private void craftIfValid(Block block, boolean craftOnce) {
        ...
        if (Constants.isSoulJarsInstalled && sfCatalyst != null
            && sfCatalyst.getId().startsWith("1FILLED") && sfCatalyst.getId().endsWith("1SOUL_JAR")) {
