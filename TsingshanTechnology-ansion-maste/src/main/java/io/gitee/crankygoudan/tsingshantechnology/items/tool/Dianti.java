...
public final class Dianti extends SlimefunItem implements Listener {

    private final BiMap<Location, Location> locations = HashBiMap.create();

    public Dianti(ItemGroup category, SlimefunItemStack item, RecipeType recipeType, ItemStack[] recipe) {
        ...
        addItemHandler(new BlockBreakHandler(false, false) {
            ...
            public void onPlayerBreak(@Nonnull BlockBreakEvent e, @Nonnull ItemStack item, @Nonnull List<ItemStack> drops) {
                ...
                Dianti.this.locations.remove(l);
                Dianti.this.locations.inverse().remove(l);
            }
        });
        addItemHandler(new BlockTicker() {
            ...
            public void tick(Block block, SlimefunItem slimefunItem, Config config) {
                if (TsingshanTechnology.slimefunTickCount() % 8 == 0 && block.getY() > 0) {
