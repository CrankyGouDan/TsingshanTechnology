...
public abstract class XiPiaMoBan extends SlimefunItem implements InventoryBlock, EnergyNetComponent, MachineProcessHolder<CraftingOperation> {

    private static final int[] BORDER = {0,1,2,3,12, 30, 36,37,38,39};
    private static final int[] BORDER_IN = {9, 10, 11, 18, 20, 27, 28, 29};
    ...
    protected XiPiaMoBan(ItemGroup category, SlimefunItemStack item, RecipeType recipeType, ItemStack[] recipe) {
    ...
    @ParametersAreNonnullByDefault
    protected XiPiaMoBan(ItemGroup itemGroup, SlimefunItemStack item, RecipeType recipeType, ItemStack[] recipe, ItemStack recipeOutput) {
    ...
    protected void constructMenu(BlockMenuPreset preset) {
        ...
        for (int i : BORDER_IN) {
            preset.addItem(i, new io.github.thebusybiscuit.slimefun4.libraries.dough.items.CustomItemStack (Material.PINK_STAINED_GLASS_PANE, Y.colorPseudorandomString ( "植入各种芯片")), ChestMenuUtils.getEmptyClickHandler());
        }

        for (int i : BORDER_OUT) {
            preset.addItem(i, new io.github.thebusybiscuit.slimefun4.libraries.dough.items.CustomItemStack (Material.MAGENTA_STAINED_GLASS_PANE,  Y.colorPseudorandomString ("输出芯片产物")), ChestMenuUtils.getEmptyClickHandler());
        }

        preset.addItem(21, new CustomItemStack(Material.BLACK_STAINED_GLASS_PANE,  Y.colorPseudorandomString ("智能机器人：一头呆呆呆呆"), Y.colorPseudorandomString ("为你服务!")), ChestMenuUtils.getEmptyClickHandler());
     ...
     */
    public final XiPiaMoBan setCapacity(int capacity) {
     ...
     */
    public final XiPiaMoBan setProcessingSpeed(int speed) {
     ...
     * @return This method will return the current instance of {@link AContainer}, so that can be chained.
     */
    public final XiPiaMoBan setEnergyConsumption(int energyConsumption) {
