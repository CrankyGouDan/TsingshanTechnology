...
/**
 * The {@link QingShanQuanXi} is a very simple block which allows the {@link Player}
 ...
 *
 */
public class QingShanQuanXi extends SlimefunItem implements HologramOwner {

    private static final String OFFSET_PARAMETER = "offset";

    @ParametersAreNonnullByDefault
    public QingShanQuanXi(ItemGroup itemGroup, SlimefunItemStack item, RecipeType recipeType, ItemStack[] recipe, ItemStack recipeOutput) {
    ...
    private @Nonnull BlockPlaceHandler onPlace() {
        return new BlockPlaceHandler(false) {
            ...
            public void onPlayerPlace(BlockPlaceEvent e) {
                ...
                BlockStorage.addBlockInfo(b, "text",  "输入你想要的内容" );
                BlockStorage.addBlockInfo(b, OFFSET_PARAMETER, "1.0");
    ...
    private void openEditor(@Nonnull Player p, @Nonnull Block projector) {
        ChestMenu menu = new ChestMenu(Slimefun.getLocalization().getMessage(p, Y.colorPseudorandomString("青山全息文字投影仪" )));

        menu.addItem(0, new CustomItemStack(Material.WRITABLE_BOOK, Y.colorRandomString ( "展示文本 (点击编辑)") + " " + Y.colorRandomString(" ") + Y.colorRandomString ( BlockStorage.getLocationInfo ( projector.getLocation (), "text" ) )));
        menu.addMenuClickHandler(0, (pl, slot, item, action) -> {
            ...
            ChatUtils.awaitInput(pl, message -> {
                ...
                hologram.setCustomName(Y.colorPseudorandomString (message));
        ...
        menu.addItem(1, new CustomItemStack(Material.BOOK, Y.colorRandomString("高度:  ") + NumberUtils.reparseDouble(Double.valueOf(BlockStorage.getLocationInfo(projector.getLocation(), OFFSET_PARAMETER)) + 1.0D), "",
            Y.colorPseudorandomString("左键单击: +0.1"),
            Y.colorPseudorandomString("右键单击: -0.1")));
